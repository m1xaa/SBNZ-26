...
## Local Context and External References

Before implementing, inspect the existing project/specification located relative to this backend work directory at:

```text
../Clash-Royale-Deck-Chooser
```

Use that project/specification as the source of truth for the domain requirements, expected behavior, terminology, and any already defined Clash Royale Deck Chooser concepts. Adapt the backend implementation to that specification instead of inventing an unrelated model.

If Drools examples or course exercises are needed, use the local exercises downloaded at:

```text
../../Vežbe
```

Use those exercises as references for Drools setup, KieContainer/KieSession usage, facts, DRL rules, rule firing, and session disposal. Do not copy blindly; adapt the patterns to this backend.

The implementation does **not** need to mirror the exact structure of `../Clash-Royale-Deck-Chooser`. A single Spring Boot project is sufficient, but it must follow the required Drools concepts: facts, DRL rules, KieSession insertion, rule execution, result collection, and disposal of the session.
...
A full multi-module or highly formal project structure is **not required**. A single Spring Boot backend project is enough, as long as the implementation is clean and Drools usage is real and visible.

The code should still keep the main responsibilities reasonably separated. A simple structure such as this is acceptable:
...
Do not spend time copying the exact structure of another project unless it directly helps the implementation. Prioritize a working Spring Boot + JPA + Drools solution.

Drools-specific code must be separated from normal service/entity code enough that it is clear where facts, rules, results, and KieSession execution are handled.

Recommended Drools package structure, if useful:
...
A simulation run represents one generated test scenario that is saved to the database and can later be replayed or inspected.
...
A simulation match is one generated match inside a simulation run. It should be persisted and then converted into Drools facts/events.
...
A simulation match event is a persisted event that should also be inserted into the Drools session as a match event fact.
...
Simulation events should cover the same reasoning cases as normal `MatchEvent` records: air losses, overcommit, heavy deck losses, lost tower, lost control, high elixir spend, and match finished events.
...
In addition to the recommendation endpoint, implement simulations that generate realistic test data, persist it to the database, and use the generated data as Drools events/facts.

Minimum required simulation behavior:

1. Create one or more simulation scenarios from the project specification in `../Clash-Royale-Deck-Chooser`.
2. Generate a player profile, owned cards, match history, and ordered match events for each scenario.
3. Persist the generated simulation run, matches, and events to PostgreSQL.
4. Convert persisted simulation matches/events into Drools facts.
5. Insert those facts into `KieSession`.
6. Fire the rules and produce a deck recommendation.
7. Persist the recommendation and link it back to the simulation run.
8. Return the simulation result through an API response so it can be inspected.
...
- Fast cycle player with high-level Hog Rider and several cheap support cards.
- Player with repeated losses against air archetypes.
- Player who repeatedly overcommits elixir and then loses tower/control.
- Player who loses often with heavy decks, so beatdown priority should decrease.
- Player with excluded cards, requiring replacement logic.
- Player with locked key cards, requiring equivalent alternatives.

Simulation data must not be only in-memory. It should be written to the database first, then loaded/mapped into facts/events used by Drools. This makes the Drools event reasoning auditable and easier to test.
...
The implementation must visibly use Drools infrastructure rather than replacing it with Java `if/else` logic. At minimum, there must be:

- a `KieContainer` or equivalent Drools configuration bean
- a service that creates a `KieSession`
- insertion of player, card, match, and event facts
- insertion of a mutable result object
- `fireAllRules()` or the appropriate CEP/event-session equivalent
- extraction of rule output from the result object
- `dispose()` in a `finally` block

Use the local Drools exercises at `../../Vežbe` if setup details are needed.
...
For simulations, the generated events should be persisted as `SimulationMatchEvent` or equivalent database rows before being inserted into Drools. The rule flow should make it clear that persisted simulation events are mapped into event facts and then used by the CEP/match-history rules.
...
Persist recommendation history and simulation history. Simulation runs must write generated matches and ordered match events to the database before using them for Drools reasoning.
...
- simulation event mapping into Drools facts
- simulation replay from persisted database records

Prefer testing Drools behavior separately from controllers.

## Coding Rules

- A single Spring Boot project is sufficient; do not over-optimize folder structure.
- Keep enough controller/service/repository separation that the implementation remains understandable.
...
- Use real Drools facts and DRL rules for reasoning; do not replace the expert system with Java-only conditional logic.
- Persist simulation runs, matches, events, and resulting recommendations.
- Use `../Clash-Royale-Deck-Chooser` as the project/specification reference.
- Use `../../Vežbe` as a local Drools/KieSession reference if needed.
...
12. Simulation generation and persistence.
13. Simulation event mapping into Drools facts/events.
14. Admin CRUD if needed.
15. CEP/event-based improvements.
16. Rule templates.
